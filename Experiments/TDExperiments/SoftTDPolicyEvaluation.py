...
def soft_policy_evaluation_experiment(cfg):
    ...
    for kp, kd, ki, alpha, beta in zip(cfg['kp'], cfg['kd'], cfg['ki'], cfg['alpha'], cfg['beta']):
        agent, env, policy = build_agent_and_env(("soft TD", kp, ki, kd, alpha, beta), cfg['env'], cfg['get_optimal'], cfg['seed'], cfg['gamma'])
        V_pi = find_Vpi(env, policy)
        test_function = build_test_function(cfg['norm'], V_pi)
        history = agent.estimate_value_function(num_iterations=cfg['num_iterations'], test_function=test_function)
