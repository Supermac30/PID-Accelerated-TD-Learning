...
def TD_comparison_experiment(cfg):
    ...
    TDagent, env, policy = build_agent_and_env(("TD", 1, 0, 0, 0, 0), cfg['env'], cfg['get_optimal'], seed, cfg['gamma'])
    ...
    save_array(TD_history, f"Regular TD", plt, cfg['normalize'])
    for kp, kd, ki, alpha, beta in zip(cfg['kp'], cfg['kd'], cfg['ki'], cfg['alpha'], cfg['beta']):
        PID_TDagent, env, policy = build_agent_and_env(("TD", kp, ki, kd, alpha, beta), cfg['env'], cfg['get_optimal'], seed, cfg['gamma'])
        ...
        save_array(PID_TD_history, f"kp={kp} kd={kd} ki={ki} alpha={alpha} beta={beta}", plt, cfg['normalize'])
