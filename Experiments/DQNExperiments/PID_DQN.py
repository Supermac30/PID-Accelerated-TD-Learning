...
@hydra.main(version_base=None, config_path="../../config/DQNExperiments", config_name="PID_DQN")
def control_experiment(cfg):
    ...
    for kp, kd, ki, alpha, beta, optimizer, \
        ...
        target_net_update_steps, epsilon, epsilon_decay, \
        ...
        zip(cfg['kp'], cfg['kd'], cfg['ki'], cfg['alpha'], cfg['beta'], cfg['optimizer'], \
            ...
            cfg['target_net_update_steps'], cfg['epsilon'], cfg['epsilon_decay'], \
            cfg['epsilon_min'], cfg['epsilon_decay_step'], cfg['train_step']):
        agent, _ = build_PID_DQN(
            kp, ki, kd, alpha, beta,
            cfg['env_name'], cfg['gamma'], optimizer,
            replay_memory_size, batch_size, learning_rate,
            target_net_update_steps, epsilon, epsilon_decay,
            epsilon_min, epsilon_decay_step, train_step
        )
        history, _ = agent.rollout(num_iterations=cfg['num_iterations'])
