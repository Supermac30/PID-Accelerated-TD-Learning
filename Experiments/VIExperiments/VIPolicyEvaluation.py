...
def policy_evaluation_experiment(cfg):
    ...
    for kp, kd, ki, alpha, beta in zip(cfg['kp'], cfg['kd'], cfg['ki'], cfg['alpha'], cfg['beta']):
        agent, env, policy = build_agent_and_env(("VI", kp, ki, kd, alpha, beta), cfg['env'], False, cfg['seed'], cfg['gamma'])
        history, _ = agent.policy_evaluation(
            num_iterations=cfg['num_iterations'],
            test_function=build_test_function(cfg['norm'], find_Vpi(env, policy))
        )
