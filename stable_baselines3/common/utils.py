...
def get_schedule_fn(value_schedule): #(value_schedule: Union[Schedule, float]) -> Schedule:
    """
...
def get_linear_fn(start, end, end_fraction): #(start: float, end: float, end_fraction: float) -> Schedule:
    """
...
def constant_fn(val): #(val: float) -> Schedule:
    """
...
def check_for_correct_spaces(env, observation_space, action_space): #(env: GymEnv, observation_space: spaces.Space, action_space: spaces.Space) -> None:
    """
...
def obs_as_tensor(obs, device): #(obs: Union[np.ndarray, Dict[str, np.ndarray]], device: th.device) -> Union[th.Tensor, TensorDict]:
    """
...
def should_collect_more_steps(train_freq, num_collected_steps, num_collected_episodes):  #( train_freq: TrainFreq, num_collected_steps: int, num_collected_episodes: int, ) -> bool:
    """
