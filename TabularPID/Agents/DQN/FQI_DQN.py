...
SelfDQN = TypeVar("SelfDQN", bound="PID_FQI")

class PID_FQI(OffPolicyAlgorithm):
    """
    FQI Deep Q-Network (DQN)
    ...
    def _on_step(self) -> None:
        ...
        if self._n_calls % max(self.target_update_interval // self.n_envs, 1) == 0:
            ...
            # Randomly reinitalize the Q network
    ...
    def train(self, gradient_steps: int, batch_size: int = 100) -> None:
        ...
        replay_data = self.replay_buffer.sample(self.replay_buffer.buffer_size, env=self._vec_normalize_env)  # type: ignore[union-attr]

        # Calculate the new BRs and zs
        with th.no_grad():
            ...
            target_current_q_values = self.q_net(replay_data.observations)
            target_current_q_values = th.gather(target_current_q_values, dim=1, index=replay_data.actions.long())

            self.BRs = target_q_values - target_current_q_values
            new_zs = beta * replay_data.z + alpha * self.BRs

            self.replay_buffer.update(replay_data.indices, zs=new_zs, BRs=self.BRs)
